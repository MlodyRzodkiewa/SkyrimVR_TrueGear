...
## C++ standalone example (SkyrimVR-friendly)
A minimal C++ program that sends a short buzz to a few motors. It connects directly to the Truegear_player WebSocket server, so you can embed or call it from a SkyrimVR mod or any native app.

### Dependencies
- CMake 3.15+
- Boost (headers + `libboost-system`)
- [websocketpp](https://github.com/zaphoyd/websocketpp) headers (package name `libwebsocketpp-dev` on Debian/Ubuntu)

### Build and run with CMake
```bash
cd Examples/TrueGearCppSample
cmake -S . -B build
cmake --build build
./build/truegear_cpp_sample ws://127.0.0.1:18233/v1/tact/
```
Press **Enter** to trigger a brief vibration on motors 0, 4, and 8, or type `quit` to exit. Adjust the WebSocket URL argument if your Truegear_player instance is exposed elsewhere.

### Build and run with Visual Studio (Windows)
1. Install dependencies via [vcpkg](https://github.com/microsoft/vcpkg) or another package manager so Boost, Asio, and websocketpp headers are available.
2. Open `Examples/TrueGearCppSample/TrueGearCppSample.sln` in Visual Studio 2022.
3. Select the **x64** platform and either the **Debug** or **Release** configuration.
4. Build and run `TrueGearCppSample`. The project links against `ws2_32` and inherits include paths from Vcpkg integration by default; adjust the include directories if you keep your headers elsewhere.
